...
<script>
import { getBlogInfo, createBlog, updateBlog } from "@/api/bgmsb";
import { fetchInfo } from "@/api/bgmscla";
import { fetchList } from "@/api/bgmscla";
...
export default {
  ...
  methods: {
    ...
    submit(isS) {
      ...
      if(this.isEdit){
        fetchInfo(this.blog.id).then((response) => {
